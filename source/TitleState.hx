...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		#if (desktop && MODS_ALLOWED)
		var path = "mods/" + Paths.currentModDirectory + "/images/logoBumpin-Ristar.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "mods/images/logoBumpin-Ristar.png";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "assets/images/logoBumpin-Ristar.png";
		}
		//trace(path, FileSystem.exists(path));
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('OldPsychLogo'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 5:
					#if PSYCH_WATERMARKS
					createCoolText(['Exists Because Of'], -40);
					#else
					createCoolText(['Exists Because Of'], -40);
					#end
				case 7:
